...
import { getHost } from "requests/request"
Vue.use(Vuex)
	...
	mutations: {
		...
		WEBSOCKET_INIT (state, url) {
		            ...
		            state.socketTask = uni.connectSocket({
		                ...
		                success: (res=> {
		                    console.log(url + ' websocket正在连接中……')
		                }),
						fail: (r=>{
							console.log(r)
						})
		            ...
		            state.socketTask.onOpen((res) => { 
		                ...
		                state.socketTask.onMessage(res => {
		                    ...
		                    console.log(data)
							state.recsocket.push(data)
							console.log(state.recsocket)
		                });
		            })
					uni.onSocketError((res) => {
						...
						that.dispatch('CLOSE_SOCKET')
						this.reTimer = setTimeout(()=>{
											//重新连接websocket,连接成功后关闭重新链接对象
											that.dispatch('WEBSOCKET_INIT', 'ws://'+ getHost() +'socket')
										},10000)
					})	
		        },
